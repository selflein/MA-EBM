...
def init_random(buffer_size, data_shape):
    return torch.FloatTensor(buffer_size, *data_shape).uniform_(-10, 10)


class EntropyMCMC(OODDetectionModel):
    def __init__(
        self,
        arch_name,
        arch_config,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.pxsgld > 0:
            x_q = self.sample_q(self.replay_buffer, n_steps=self.sgld_steps)

            _, logits = self.model(x_q, return_logits=True)
            p_y_x = torch.log_softmax(logits, 1)
            l_pxsgld = (p_y_x.exp() * p_y_x).sum(1).mean()

            l_pxsgld *= self.pxsgld
            self.log("train/entropy_loss", l_pxsgld)
    ...
    def validation_epoch_end(self, outputs):
        if self.is_toy_dataset:
            ...
            energy, logits = self.model(data, return_logits=True)
            px = to_np(energy.exp())
            conf = to_np(torch.softmax(logits, 1).max(1).values)
            ...
            fig, ax = plt.subplots()
            mesh = ax.pcolormesh(x, y, conf.reshape(*x.shape))
            fig.colorbar(mesh)
            samples = to_np(self.replay_buffer)
            ax.scatter(samples[:, 0], samples[:, 1])
            self.logger.experiment.add_figure(
                "dist/confidence", fig, self.current_epoch
            )
            plt.close()
    ...
    def sample_q(self, replay_buffer, y=None, n_steps=20, contrast=False):
        ...
        for _ in range(n_steps):
            _, logits = self.model(x_k, y=None, return_logits=True)
            p_y_x = torch.log_softmax(logits, 1)
            energy = (p_y_x * p_y_x.exp()).sum(1).sum()
